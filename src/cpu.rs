...
enum Instr {                                       // instruction in hexadecimal
    ...
    JMP { x: Memory },                             // 1x-xx
    JSR { x: Memory },                             // 2x-xx
    SKEQ { x: Register, k: u8 },                   // 3x kk
    SKNE { x: Register, k: u8 },                   // 4x kk
    SKEQREG { x: Register, y: Register },          // 5x y0
    MOV { x: Register, k: u8 },                    // 6x kk
    ADD { x: Register, k: u8 },                    // 7x kk
    MOVREG { x: Register, y: Register },           // 8x y0
    OR { x: Register, y: Register },               // 8x y1
    AND { x: Register, y: Register },              // 8x y2
    XOR { x: Register, y: Register },              // 8x y3
    ADDCARRY { x: Register, y: Register },         // 8x y4
    SUB { x: Register, y: Register },              // 8x y5
    SHR { x: Register, y: Register },              // 8x y6
    RSB { x: Register, y: Register },              // 8x y7
    SHL { x: Register, y: Register },              // 8x ye
    SKNEREG { x: Register, y: Register },          // 9x y0
    MVI { x: Memory },                             // ax-xx
    JMI { x: Memory },                             // bx-xx
    RAND { x: Register, k: u8 },                   // cx kk
    SPRITE { x: Register, y: Register, height: usize }, // dx ys
    SKPR { x: Register },                          // ex 9e
    SKUP { x: Register },                          // ex a1
    GDELAY { x: Register },                        // fx 07
    KEY { x: Register },                           // fx 0a
    SDELAY { x: Register },                        // fx 15
    SSOUND { x: Register },                        // fx 18
    ADDINDEX { x: Register },                      // fx 1e
    FONT { x: Register },                          // fx 29
    BCD { x: Register },                           // fx 33
    STR { x: Register },                           // fx 55
    LDF { x: Register },                           // fx 65
}
// @formatter:on
...
impl Cpu {
    ...
    pub fn cycle(&mut self, s: f64) {
        ...
        for _ in 0..num_instructions {
            ...
            let instruction = self.fetch_instruction()
                .unwrap_or_else(|| panic!("Illegal instruction: 0x{:02x?} 0x{:02x?}. PC: {:#x?}", self.memory[self.pc], self.memory[self.pc + 1], self.pc));
    ...
    fn fetch_instruction(&self) -> Option<Instr> {
        ...
        match mask_xo(h) {
            ...
            0x30 => Some(SKEQ { x: mask_ox(h), k: l }),
            0x40 => Some(SKNE { x: mask_ox(h), k: l }),
            0x50 => Some(SKEQREG { x: mask_ox(h), y: shift_right_xo(l) }),
            0x60 => Some(MOV { x: mask_ox(h), k: l }),
            0x70 => Some(ADD { x: mask_ox(h), k: l }),
            0x80 => match mask_ox(l) {
                0x00 => Some(MOVREG { x: mask_ox(h), y: shift_right_xo(l) }),
                0x01 => Some(OR { x: mask_ox(h), y: shift_right_xo(l) }),
                0x02 => Some(AND { x: mask_ox(h), y: shift_right_xo(l) }),
                0x03 => Some(XOR { x: mask_ox(h), y: shift_right_xo(l) }),
                0x04 => Some(ADDCARRY { x: mask_ox(h), y: shift_right_xo(l) }),
                0x05 => Some(SUB { x: mask_ox(h), y: shift_right_xo(l) }),
                0x06 => Some(SHR { x: mask_ox(h), y: shift_right_xo(l) }),
                0x07 => Some(RSB { x: mask_ox(h), y: shift_right_xo(l) }),
                0x0E => Some(SHL { x: mask_ox(h), y: shift_right_xo(l) }),
                _ => None
            },
            0x90 => Some(SKNEREG { x: mask_ox(h), y: shift_right_xo(l) }),
            0xA0 => Some(MVI { x: mask_oxxx(h, l) }),
            0xB0 => Some(JMI { x: mask_oxxx(h, l) }),
            0xC0 => Some(RAND { x: mask_ox(h), k: l }),
            0xD0 => Some(SPRITE { x: mask_ox(h), y: shift_right_xo(l), height: mask_ox(l) }),
            0xE0 => match l {
